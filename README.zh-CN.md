...
- 项目概览仍保留成员协作快照与部分展示文案的本地补充数据；`/project/:projectId/resources` 中 `agents` 仍使用本地目录 fallback。
- `/knowledge`、`/skills`、`/agents` 已切到正式后端资产接口；其中 `/skills` 已支持原生 `SKILL.md` 自建、GitHub/URL 导入、预览、草稿/发布与删除，项目内 Skill 元数据也已优先消费 `/api/skills`。
