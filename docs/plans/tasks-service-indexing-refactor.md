...
任务：排除 `.codex/`、legacy `.agent` 目录、`files/`、`resume/` 等非构建输入；把 Docker 说明收口到单一主入口，其余文档改链接引用。
...
- 2026-03-18：Milestone 6 收尾（`SR-62` 完成）
  - `.dockerignore` 已新增 `.codex/`、legacy `.agent` 目录、`files/`、`resume/` 以及 nested `coverage / dist / build` 等非构建输入排除规则，避免根 compose build context 继续把协作文档、知识模板和历史产物打进镜像上下文。
