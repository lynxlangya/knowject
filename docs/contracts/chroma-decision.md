...
状态：部分实施（截至 2026-03-19，`global_docs` 已打通 Node -> Python FastAPI 控制面 -> OpenAI-compatible / settings-driven embedding -> Chroma 写入与 Node 统一检索闭环；文档级 rebuild、知识库级 rebuild 的 Node 编排、diagnostics、文档 / 知识库级向量 delete，以及项目私有 knowledge write-side 均已落地；项目级 merged retrieval、对话 runtime 最小消费与默认流式发送已落地；`global_code` 仍只做 collection 预留，更完整的来源引用渲染、流式恢复体验与 Skill / Agent runtime 仍未落地）；当前事实仍以 `docs/current/architecture.md` 为准，本文件负责固定 Knowject 使用 Chroma 的角色定位、阶段边界与推荐实现约束。
...
- 与其他文档的关系：
  - 当前事实以 `docs/current/architecture.md` 为准。
  - Week 3-4 的阶段计划以 `docs/plans/tasks-global-assets-foundation.md` 为准。
  - Week 5-6 的阶段计划以 `docs/plans/tasks-index-ops-project-consumption.md` 为准。
  - 长期目标以 `docs/roadmap/target-architecture.md` 为准。
...
- 当前仓库已经正式接入：
  ...
  - 文档级 rebuild、知识库级 rebuild 的 Node 编排，以及 diagnostics
  - 文档 / 知识库级向量 delete
...
- 当前 diagnostics 契约已拆分为“Python indexer 运行时实际值”与“工作区期望值”：前者通过 `indexer.supportedFormats / chunkSize / chunkOverlap / embeddingProvider` 返回，后者通过 `indexer.expected.*` 单独暴露。
- 当前仓库尚未正式接入：
  - `global_code` 真实导入链路
- 当前仍处于过渡态的实现：
  - 向量 delete 的正式 Python 内部接口已经补齐，Node 代码仅对旧版 indexer 保留 404 回退 delete 路径
- 当前迭代重点已经从 Week 3-4 的“全局资产正式化”转向“更完整的来源引用渲染、流式体验收口与 Skill / Agent runtime 的收口”；本文件只继续固定索引分层、namespace / collection 与检索边界，不把未来运行时能力写成已落地事实。
...
| `apps/indexer-py`（已落地基线） | Python 独立索引控制面 / worker；当前已采用 FastAPI + `uv`，负责 parse / clean / chunk / embed / upsert / delete，并已提供 document rebuild、document / knowledge delete 与 diagnostics 内部入口；`retry` 与 knowledge rebuild 继续由 Node 业务层编排 | 本阶段已落为最小独立运行时，不要求一上来做分布式队列 |
...
- 向量 delete 的正式 Python 内部接口已经补齐，Node 代码里保留的只是不支持新版路由时的 404 回退 delete 路径；这不改变职责归属。
...
- `rebuild knowledge`
  - 当前统一按 namespace 级重建编排，而不是“同 fingerprint 只在原 collection 混写”。
  - 无论 active embedding fingerprint 是否变化，都必须先把当前 namespace 的文档重建到新的 target collection；若默认 versioned 名与 active pointer 相同，则服务端必须派生临时 `stage_*` 后缀，避免直接污染旧 active collection。
  - 只有当 target collection 全部写入成功后，才允许切换 active pointer；失败时必须保留旧 active collection 可继续检索。
...
- 当前仍然冻结的基础契约：
  ...
  - 业务状态回写：Node 独占写入权
- 当前不再冻结为单一 provider / model：
  - 运行时 embedding provider / baseUrl / model 由 `/api/settings` 的 effective config 决定，缺失时才回退到环境变量
  - 开发环境缺少 `OPENAI_API_KEY` 时允许退化为 deterministic 本地 embedding，用于保证上传 / 检索闭环可验证
