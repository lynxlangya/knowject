...
- 样式常量：Tailwind canonical class 统一、radius/text/shadow token 已在 repo 中形成基线，可封装到主题 helper 供 `packages/ui` 与 `apps/platform` 复用，保持命名与现有视觉系统一致。
...
- 经典情形：`Apps/platform` 侧栏、project 表单等已经具有明确结构的地方，过早做 “超级通用组件” 反而增加耦合并让调试变得困难，Plans 一直强调“先保持可读再抽象”的原则。
...
- 复查 Tailwind canonical / token 做法，确保新的 helpers 与 `packages/ui`/`apps/platform` 中现有视觉系统保持一致且安全 canonical 前缀已被统一。
...
## 7. 文档同步要求
- 抽象治理变动后，更新 `docs/current/architecture.md` 中的前端架构/路由描述，确保抽象后的组件层级仍能被理解。
- 若新抽象影响 `packages/ui` 的共享组件/样式，需要同时更新 `docs/standards/code-structure-governance.md` 或 `core-code-commenting` 中对应条目，并通知页面 owner。
