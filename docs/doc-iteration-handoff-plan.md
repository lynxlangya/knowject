...
先以 `docs/architecture.md` 为事实锚点，补齐当前文档里遗漏但源码已落地的业务逻辑。  
然后新增两份衔接文档：一份负责“人和 AI 怎么快速接手”，一份负责“把上下文准确交给下一位”。  
最后更新 `docs/README.md` 和仓库根 `README.md`，让协作者从入口就能选对文档，而不是在多个文档之间来回猜。
...
- 预期结果：新接手者能在单个 `docs/` 目录内找到事实、计划、交接入口
...
3. 需要同步哪些 README / AGENTS / docs：
   - 同步 `docs/README.md` 与根 `README.md`
   - 本次不需要更新 `AGENTS.md`
4. 预期产出是什么：
   - 一份本轮执行计划
   - 一份快速接手指南
...
- 手动验证：
  - 检查 `docs/README.md` 中的阅读顺序、角色表和维护边界是否覆盖新增文档
  - 检查 `docs/architecture.md` 中新增业务事实是否都能在源码中找到对应证据
- 文档一致性检查：
  - 确认 `README.md`、`docs/README.md`、`docs/handoff-guide.md` 对新增文档的入口描述一致
- 最小验收结果：
  - 新接手者可以只依赖 `docs/` 下文档建立当前事实，并知道下一步该读什么、改什么、验什么
...
- 风险：
  ...
  - 入口文档新增过多后反而提高阅读负担
- 回滚方式：
  - 删除新增 handoff 文档，并把入口回退到 `docs/architecture.md + docs/README.md`
- 是否可重试：
  - 可重试，且属于纯文档回滚
...
- 决策：
  - 把执行计划也沉淀到 `docs/`，而不是只留在回复中
- 原因：
  - 用户明确要求执行计划和高价值文档都要沉淀，且后续接手者需要看到“为什么要改这些文档”
- 备选方案：
  - 使用 `.agent/PLANS.md` 仅保留模板，不新增本轮计划文档
