...
import { SKILL_ENTRY_FILE_NAME, assertSafeBundleRelativePath } from './skills.shared.js';
import type { ImportSkillInput, SkillImportProvenance } from './skills.types.js';
...
type NormalizedImportInput = NormalizedUrlImportInput | NormalizedGitHubImportInput;
...
const createImportFetchError = (message: string, details?: unknown): AppError => {
  return new AppError({
    statusCode: 400,
...
const readOptionalBooleanField = (value: unknown, field: string): boolean | undefined => {
  if (value === undefined) {
    return undefined;
  }
...
const fetchBuffer = async (url: string): Promise<{ buffer: Buffer; contentType: string | null }> => {
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'knowject-skill-import',
    },
  ...
  if (!response.ok) {
    throw createImportFetchError(`Skill 资源下载失败（HTTP ${response.status}）`, {
      url,
      status: response.status,
    });
  }
...
const fetchText = async (url: string): Promise<{ text: string; contentType: string | null }> => {
...
const normalizeImportInput = (input: ImportSkillInput): NormalizedImportInput => {
  ...
  const mode = readOptionalStringField(input.mode, 'mode');
  const dryRun = readOptionalBooleanField(input.dryRun, 'dryRun') ?? false;

  if (mode === 'url') {
    const url = readOptionalStringField(input.url, 'url');
  ...
  if (mode === 'github') {
    const githubUrl = readOptionalStringField(input.githubUrl, 'githubUrl');
    ...
    const repository = readOptionalStringField(input.repository, 'repository');
    if (!repository) {
    ...
    const path = normalizeBundleRootPath(
      readOptionalStringField(input.path, 'path') ?? undefined,
    );
    const ref = readOptionalStringField(input.ref, 'ref') ?? null;
...
const buildGitHubContentsUrl = (target: GitHubImportTarget, bundleRootPath: string): string => {
...
const readGitHubDirectoryEntries = async (
  ...
  if (!Array.isArray(response)) {
    throw createImportFetchError('GitHub 路径必须指向 Skill 目录或 SKILL.md 文件', {
      url,
    });
  }
  ...
  return response.filter(
    (item): item is GitHubContentItem =>
      Boolean(
        item &&
          typeof item === 'object' &&
          ('type' in item || 'download_url' in item || 'path' in item || 'url' in item),
      ),
  ...
  const walkEntries = async (entries: GitHubContentItem[]): Promise<void> => {
    for (const entry of entries) {
      ...
      const relativePath = assertSafeBundleRelativePath(
        bundleRootPath ? posix.relative(bundleRootPath, entry.path) : entry.path,
      );
...
const importFromGitHub = async (
  ...
  const skillFile = bundleFiles.find((file) => file.path === SKILL_ENTRY_FILE_NAME);
...
const importFromUrl = async (url: string): Promise<ImportedSkillBundle> => {
  ...
  if (contentType?.includes('text/html') || looksLikeHtmlDocument(text)) {
    throw createImportFetchError('URL 导入只支持原始 Markdown 文本，不支持网页地址', {
      url,
      contentType,
    });
  }
