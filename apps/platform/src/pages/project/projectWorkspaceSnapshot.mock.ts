...
const PROJECT_META_BY_ID: Record<
  string,
  Pick<ProjectWorkspaceMeta, "iconUrl">
> = {
  "project-mobile-rebuild": {
...
const buildDefaultMemberSnapshot = (index: number): ProjectMemberSnapshot => {
  ...
  return (
    defaultSnapshots[index] ?? defaultSnapshots[defaultSnapshots.length - 1]
  );
};
...
export const getProjectWorkspaceSnapshot = (
  ...
  return {
    ...
    stats: {
      ...
      agentCount: AGENTS_FEATURE_ENABLED ? project.agentIds.length : 0,
