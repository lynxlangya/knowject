...
export const ProjectResourceGroup = ({
  ...
  const renderResourceCard = (item: ProjectResourceItem) => (
    <article
      ...
      onKeyDown={
        onItemClick ? (event) => handleItemKeyDown(event, item) : undefined
      }
    >
      ...
      <div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-slate-500">
        ...
        <span>
          {tp("resources.group.updatedAt", { value: item.updatedAt })}
        </span>
        {item.type === "knowledge" ? (
          <span>
            {tp("resources.group.documentCount", {
              count: item.documentCount ?? 0,
            })}
          </span>
        ) : (
          <span>
            {tp("resources.group.usageCount", { count: item.usageCount })}
          </span>
        )}
      </div>
    </article>
  ...
  return (
    <section
      className={[
        ...
        highlighted ? "border-[#C2EDE6] bg-[#F2FDFB]/60" : "border-slate-200",
      ].join(" ")}
    >
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          {showOpenGlobal ? (
            <Tooltip
              title={tp("resources.group.viewGlobal", { title: group.title })}
            >
              <Button
                aria-label={tp("resources.group.viewGlobal", {
                  title: group.title,
                })}
                icon={<ExportOutlined />}
