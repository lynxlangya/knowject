...
test("tag copy follows runtime locale changes after module initialization", async () => {
  ...
  const { ProjectResourceGroup } =
    await import("../src/pages/project/components/ProjectResourceGroup");
  ...
  const { AGENT_STATUS_META } =
    await import("../src/pages/agents/constants/agentsManagement.constants");
  const { CATEGORY_META, STATUS_META } =
    await import("../src/pages/skills/constants/skillsManagement.constants");
  ...
  const englishKnowledgeDocument =
    KNOWLEDGE_DOCUMENT_STATUS_META.completed.label;
  const englishAgentStatus = AGENT_STATUS_META.active.label;
  const englishSkillCategory = CATEGORY_META.documentation_architecture.label;
  ...
  assert.notEqual(
    KNOWLEDGE_SOURCE_TYPE_META.global_docs.label,
    englishKnowledgeSource,
  );
  assert.notEqual(
    KNOWLEDGE_INDEX_STATUS_META.completed.label,
    englishKnowledgeIndex,
  );
  assert.notEqual(
    KNOWLEDGE_DOCUMENT_STATUS_META.completed.label,
  ...
  assert.equal(
    CATEGORY_META.documentation_architecture.label,
    skillsTp("category.documentation_architecture"),
  );
  assert.equal(STATUS_META.draft.label, skillsTp("status.draftBadge"));
  assert.notEqual(
    CATEGORY_META.documentation_architecture.label,
    englishSkillCategory,
  );
