...
- `src/pages/project/ProjectChatPage.tsx`：项目对话编排层；对话配置、详情读取、流式 turn、以及 create / rename / delete 动作已分别拆到 `useProjectChatSettings.ts`、`useProjectConversationDetail.ts`、`useProjectConversationTurn.ts` 与 `useProjectChatActions.ts`，markdown 与 bubble 展示拆到 `projectChat.markdown.tsx` 与 `projectChatBubble.components.tsx`。
