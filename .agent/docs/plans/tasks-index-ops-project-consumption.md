...
- 如果要继续推进对话核心，请直接组合使用：
  ...
  - `.agent/docs/roadmap/target-architecture.md` 的 `5.3 Week 7-8 对话核心`
  - 本文的 `Week 7-8 的交接接口`
...
- 项目私有 knowledge 已完成 `scope / projectId` 模型、项目级 `list / create / detail / upload` 路由、`proj_{projectId}_docs` namespace 写侧与删除项目时的级联清理。
...
- 项目私有知识库最小闭环打通：
  ...
  - 写入 `proj_{projectId}_docs` namespace 当前 active collection，或等价的项目级命名空间
  - 在项目资源页看到正式状态与文档数量
...
- 推荐结论：
  ...
  - `global_code` 继续只保留预留
  - `proj_{projectId}_docs` 作为项目私有文档 namespace key
...
- 目标：把 Week 5-6 的“主线 / 延后项 / namespace key 命名 / scope 语义”先定死，避免做到一半再回退。
- 输出：
  - 本任务文档定稿。
...
- 已完成记录：
  ...
  - 已冻结 namespace key 命名：`global_docs`、`global_code`、`proj_{projectId}_docs`、`proj_{projectId}_code`；其中 Week 5-6 只允许真正落地 `proj_{projectId}_docs`。后续 2026-03-16 的实际代码已把物理 collection 升级为 versioned naming，但 namespace key 契约保持不变。
  - 已冻结文档格式优先级：`pdf` 仅作为不阻塞主线的有条件恢复项，`docx / Word` 不进入 Week 5-6 硬性 DoD。
- 建议子任务：
  - 明确 `projects.knowledgeBaseIds` 与项目私有 knowledge 的关系。
  - 冻结 `proj_{projectId}_docs` namespace key 命名。
...
- 已完成记录：
  ...
  - diagnostics 的期望 collection 命名已改为读取 knowledge 自身的 `scope / projectId / sourceType`，为后续 `proj_{projectId}_docs` namespace 写侧接入预留稳定 contract。
...
- 输出：
  ...
  - 项目级存储路径约定。
  - `proj_{projectId}_docs` namespace 写侧接入。
- 依赖：`IC-05`。
- 已完成记录：
  ...
  - 项目私有 knowledge 当前固定只支持 `global_docs`；上传后会把原始文件落到 `projects/{projectId}/knowledge/{knowledgeId}/{documentId}/{documentVersionHash}/{fileName}`，并写入 `proj_{projectId}_docs` namespace 当前 active 的 versioned Chroma collection。
...
- `proj_{projectId}_docs` namespace key 命名与最小上传 / 索引写侧。
...
- 风险 6：项目级 namespace / collection 契约和 metadata 过早过度设计
  - 止损：Week 5-6 只冻结 `proj_{projectId}_docs` namespace key 与最小字段，复杂多集合检索策略延后到 Week 7-8。
