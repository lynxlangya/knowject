...
- 全局知识库具备正式元数据模型、文档上传、解析、分块、向量化、索引状态展示的最短闭环。
- Chroma 与后端建立正式集成基线，`global_docs` 可写可查，`global_code` 至少完成命名空间和数据契约预留。
...
以下内容默认作为“推荐推进假设”，用于降低 Week 3-4 期间的范围漂移；若其中任何一项在正式启动前被否决，必须先回改本文件，再进入实现。
...
### D6. Embedding Provider 与环境契约

- 推荐结论：GA-05 / GA-06 开始前，必须先冻结一个可执行的 embedding provider 方案，以及最小环境变量契约；未冻结前，不应承诺真正的向量写入闭环。
- 原因：
...
- `search_documents` 需要复用统一的知识检索 service 或最小诊断查询入口，以证明 Skill 能挂到正式知识库；不应由 Skill 自己直接操作底层向量存储。
...
- 目标：
  - 打通“知识库元数据 + 文档上传 + 解析分块 + Chroma 写入 + 状态展示”。
- 预期结果：
...
- 子任务：
  ...
  - 冻结 embedding provider、最小环境变量契约和失败回退口径。
- 验收：
...
- 建议子任务：
  ...
  - 为后续文件上传和 Chroma 调用预留 service 层扩展点。
- 验收：
  - 3 组路由都能以最小占位响应工作。
  - 模块目录不把知识库、Skill、Agent 逻辑混写到 `memory` 路由里。

### GA-03 TODO · 设计并落地全局知识库元数据模型
...
- 推荐最小字段：
  ...
  - 文档：`id / knowledgeId / fileName / mimeType / storagePath / parseStatus / chunkCount / errorMessage / uploadedBy / uploadedAt / processedAt`
- 建议子任务：
  - 明确 `usedByProjects` 当前是否保留为派生字段，而非直接手写。
  - 补 Mongo 索引：名称、创建者、更新时间。
  - 明确列表接口与详情接口返回 shape。
- 验收：
  - 知识库与文档记录字段能支撑上传、状态展示和后续项目绑定。
...
- 建议子任务：
  ...
  - 处理逻辑先放在应用内异步执行，不引入外部队列。
  - 明确服务重启或任务中断后的恢复策略：长时间停留在 `processing` 的记录如何转为 `failed` 或允许重试。
- 验收：
  - 上传后状态能从 `pending` 进入 `processing` 再到 `completed` 或 `failed`。
  - 至少一种文档格式能产生稳定 chunk 结果。

### GA-06 TODO · 接入 Chroma 与 `global_docs` / `global_code` 命名空间

- 目标：把解析出来的 chunk 进入正式向量基础设施。
- 输出：
  - Chroma client 封装。
  - 集合初始化逻辑。
  - `global_docs` 写入 / 删除 / 重建能力。
  - `global_code` 集合预留和 metadata 约定。
- 依赖：`GA-05`。
- 建议子任务：
  - 在正式编码前确认 embedding provider 已冻结，避免 Chroma 接入完成后仍无法生成向量。
  - 新增 Chroma 环境变量与连接检测。
...
### GA-07 TODO · 前端 `/knowledge` 接正式接口并补齐状态视图
...
- 建议子任务：
  - 复用 `GlobalAssetManagementPage` 结构，不重做页面骨架。
...
- 风险 2：Chroma 集成超预期复杂
  - 止损：先把 `global_docs` 跑通，`global_code` 只保留集合预留；若 embedding provider 或 Chroma 依赖在 Week 3 中段仍未稳定，里程碑 1 先降级为“元数据 + 上传 + 解析 + 状态流”闭环。
- 风险 3：Skill 范围膨胀
  - 止损：只做 3 个内置 Skill；不开放用户自建。
- 风险 4：Agent 页面想一步到位做成对话入口
  - 止损：本阶段明确 Agent 只是配置资产，不做会话运行。
- 风险 5：前端为了兼容旧 Mock 而双轨过久
  - 止损：每个页面切正式数据后，及时删除对应 Mock 依赖，避免“看起来接了 API，实际上还混着旧数据”。

## 最小验收清单

- [ ] `/knowledge` 已接正式接口，至少支持创建知识库和上传文档。
- [ ] 至少一种文档类型能走完上传、解析、分块、向量化、状态更新。
- [ ] Chroma `global_docs` 已实际写入数据。
- [ ] `/skills` 已展示正式 Skill 列表，且包含 3 个内置 Skill。
- [ ] `/agents` 已支持创建并绑定知识库 / Skill。
- [ ] 后端具备 `knowledge / skills / agents` 三组正式模块。
- [ ] 至少完成一项自动化验证。
- [ ] 文档已同步，不再把目标态写成现状。
