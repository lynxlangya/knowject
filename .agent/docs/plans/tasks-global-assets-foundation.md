...
状态：截至 2026-03-14，GA-01、GA-02、GA-03、GA-04、GA-05、GA-06、GA-07、GA-08、GA-09、GA-10、GA-11、GA-12 已完成；本文件当前同时承担“Week 3-4 任务清单 + 完成记录”角色。
...
- 全局技能具备稳定 registry，并在此基础上完成 `SKILL.md` 自建、GitHub/URL 导入、草稿/发布与绑定校验的正式治理闭环，但不提前把执行 runtime 一起做完。
...
- GA-04 已完成：
  ...
  - 文档上传入口已接入，当前正式上传契约支持 `md / markdown / txt`，并把原始文件落到本地存储。
- GA-05 已完成：
  ...
  - `pdf` 已从当前前后端正式上传契约中移除，待 `indexer-py` 正式覆盖后再统一恢复。
...
- GA-08 已完成：
  - `skills` 模块已切到代码级 Skill registry，`GET /api/skills` 可稳定返回 3 个内置 Skill 的标准化定义。
  - `search_documents` 已明确复用服务端统一知识检索契约；`search_codebase / check_git_log` 当前先冻结 handler 与参数 schema，不假装有完整 runtime。
- GA-09 已完成：
  - 前端已新增 `skills` API client，并将 `/skills` 页面切到正式 `/api/skills` 数据源。
  - 页面已支持原生 `SKILL.md` 自建、GitHub/URL 导入、编辑、预览、草稿/发布与删除，并保留系统内置 Skill 的只读查看边界。
  - 页面已标注“系统内置 / 自建 / 公网导入”“运行状态 / 生命周期 / handler / 参数契约”，可支撑后续 Agent 绑定选择。
- GA-10 已完成：
  - `agents` 模块已落地 Mongo 正式模型、列表 / 详情 / 创建 / 编辑 / 删除接口，以及 `name / systemPrompt / status` 等最小字段校验。
  - 服务端已校验 `boundKnowledgeIds` 必须命中已存在知识库、`boundSkillIds` 必须命中系统内置或已发布 Skill，避免绑定脏引用。
  - 当前 `model` 字段由服务端固定写入 `server-default`，前端暂不开放配置入口，保持本阶段边界稳定。
- GA-11 已完成：
  - 前端已新增 `agents` API client，并将 `/agents` 页面切到正式 `agents / knowledge / skills` 数据源。
  - 页面已提供创建 / 编辑弹窗、删除确认与绑定多选交互，可完成“创建 Agent -> 绑定知识库 / Skill -> 列表回显”闭环。
  - 当前模型信息仍由服务端固定，页面只维护名称、描述、`systemPrompt`、状态与资源绑定，不扩到执行 runtime。
- 全局资产当前仍是部分前端 Mock：
  - `/knowledge` 已切正式接口并补状态视图。
  - `project.catalog.ts` 仍提供项目资源页与成员聚合所需的 `agents` 过渡目录。
  - `GlobalAssetManagementPage.tsx` 当前保留为历史壳层组件，`/skills` 与 `/agents` 已切到各自的专用正式管理页。
- 项目资源绑定已正式写回后端项目模型：
...
- 后端已建立 `knowledge / skills / agents` 正式模块边界；其中 `knowledge` 已具备真实元数据模型、CRUD、上传入口、`global_docs` Chroma 写入、统一检索与前端正式状态视图，`skills` 已完成 hybrid registry、正式 CRUD、GitHub/URL 导入、草稿/发布与绑定校验，`agents` 已完成后端正式模型、CRUD 与绑定校验。
- 当前还没有 `global_code` 真实导入、项目私有知识库，以及 Skill / Agent 运行时能力。
- `apps/indexer-py` 已具备可运行的 FastAPI 内部索引控制面，并已实现 embedding、Chroma upsert / delete；但对外可见的重建 / retry / diagnostics 接口仍未补齐。
- Skill 执行 runtime 以及 Agent / Skill 的真实运行链路仍未补齐。

### 当前不应重复立项

- 登录页、JWT、项目 CRUD、项目成员管理。
- `/knowledge`、`/skills`、`/agents` 的页面路由与既有信息架构。
...
- 全局 Skill 最小闭环打通：
  ...
  - 支持原生 `SKILL.md` 自建、GitHub/URL 导入、导入预览与 `draft / published` 生命周期
  - 项目与 Agent 绑定只允许系统内置或已发布的 Skill
  - 暂不要求接入真实对话链路
- 全局 Agent 最小闭环打通：
  - 创建 / 编辑 / 列表 / 详情
  - 绑定知识库与 Skill
...
- Skill 的回源同步、复杂执行型 Skill、代码执行型 Skill 沙箱。
- 模型可配置 UI。
...
- 当前结论：Week 3-4 最终已从“仅内置 Skill 注册表”扩展到“系统内置 + 自建 + GitHub/URL 导入”的 hybrid registry，并补齐 `SKILL.md` 编辑、导入预览、草稿/发布与绑定校验。
- 保持冻结的边界：
  - 仍不在本阶段实现 Skill runtime、回源同步、复杂执行沙箱与项目对话调度。
  - 仍以“治理链路先收口、运行链路后补齐”为核心取舍。

### D2. 文档接入格式

- 推荐结论：当前正式上传契约只支持 `md / markdown / txt`；`pdf` 恢复延后到 `indexer-py` 正式覆盖后再统一加回。
- 原因：
  - `docx`、网页抓取、仓库克隆会显著放大解析复杂度。
  - 先把“上传 - 解析 - 分块 - 向量化 - 状态反馈”跑通，再扩格式范围。
...
- Week 3-4 的首版曾以“代码 registry + 只读接口”为最小落地假设；截至 2026-03-14，该假设已被正式 Skill 资产治理链路 supersede。
- 内置 Skill 先固定为：
  - `search_codebase`
  - `check_git_log`
  - `search_documents`
...
- 当前上传入口正式契约只支持 `md / markdown / txt`，并通过 `50 MB` 限制、空文件校验和格式校验给出明确错误。
...
- 已明确 `pdf` 已从当前正式上传契约中移除，待 `indexer-py` 正式覆盖后再恢复，而不是继续对外暴露不稳定失败路径。
- 建议子任务：
- 先保证 `md / markdown / txt` 稳定跑通，再视 `indexer-py` 覆盖情况恢复 `pdf`。
...
### GA-08 DONE（2026-03-14）· 建立全局 Skill 注册表与 3 个内置 Skill 定义
...
- 依赖：`GA-02`、`GA-06`。
- 已完成记录：
  - 已在 `apps/api/src/modules/skills/skills.registry.ts` 落地代码级内置 Skill 注册表，稳定冻结 `search_codebase`、`check_git_log`、`search_documents` 三个内置 Skill 的 `id / handler / parametersSchema / status`。
  - 已让 `GET /api/skills` 返回正式只读列表 shape，不再输出 GA-02 占位 meta 与空数组。
  - 已明确 `search_documents` 的 handler 指向服务端统一知识检索契约，避免 Skill 直连底层 Chroma；`search_codebase / check_git_log` 当前保留 contract-only 状态，等待后续 runtime 接线。
  - 已补 `skills.service.test.ts`，覆盖稳定 ID、service-layer 契约和 contract-only 状态的最小验证。
...
### GA-09 DONE（2026-03-14）· 前端 `/skills` 接正式数据并升级为资产治理页

- 目标：让 Skill 页从“静态卡片 / 只读目录”升级为“正式 Skill 资产治理页”。
- 输出：
  - 前端 `skills` API client。
  - Skill 列表 / 详情治理视图。
  - 自建、GitHub/URL 导入、编辑、预览、草稿/发布与删除交互。
  - 类型、来源、可用状态、生命周期与 provenance 展示。
- 依赖：`GA-08`。
- 已完成记录：
  - 已新增 `apps/platform/src/api/skills.ts`，统一封装 `GET /api/skills` 并对齐正式 response shape。
  - 已新增 `apps/platform/src/pages/skills/SkillsManagementPage.tsx`，保留现有壳层语义，但改为从正式接口拉取 Skill 目录并承载治理交互。
  - 页面已补齐系统内置 / 自建 / 公网导入三类来源标识，以及类型、状态、handler、参数契约、生命周期与参数摘要展示，不再依赖 `project.catalog.ts` 的 Mock Skill 卡片。
  - 页面已接入原生 `SKILL.md` 编辑与前端预览校验、GitHub/URL 导入预览，以及发布 / 删除操作反馈。
  - 当前 `/agents` 与项目资源页仍维持既有过渡数据边界，没有在本任务中扩 scope。
- 建议子任务：
  - 维持现有页面壳层，替换数据源并补治理交互。
  - 对内置 Skill 明确标注“系统内置”，避免用户误以为可任意编辑。
  - 将自建与导入统一收口为 `SKILL.md` 事实源，避免页面层另起一套结构化编辑模型。
- 验收：
  - `/skills` 页面不再依赖 `project.catalog.ts` 的 Mock Skill 数据。
  - 页面展示的信息能支撑 Agent 绑定选择。
  - 自建 / 导入 Skill 默认以 `draft` 创建，发布后才可绑定。

### GA-10 DONE（2026-03-14）· 落地全局 Agent 模型与 CRUD
...
- 依赖：`GA-03`、`GA-08`。
- 已完成记录：
  - 已在 `apps/api/src/modules/agents` 落地 `AgentDocument` 模型、Mongo 仓储、索引策略与 response shape。
  - 已提供 `GET /api/agents`、`GET /api/agents/:agentId`、`POST /api/agents`、`PATCH /api/agents/:agentId`、`DELETE /api/agents/:agentId`。
  - 已在 service 层校验 `boundKnowledgeIds` 与 `boundSkillIds` 的存在性，返回明确验证错误而不是写入脏引用。
  - 已补 `agents.service.test.ts`，覆盖创建成功、非法 Skill 绑定、更新绑定与删除流程。
- 推荐最小字段：
...
### GA-11 DONE（2026-03-14）· 前端 `/agents` 接正式接口并完成绑定表单
...
- 已完成记录：
  - 已新增 `apps/platform/src/api/agents.ts`，统一封装 `GET /api/agents`、`POST /api/agents`、`PATCH /api/agents/:agentId` 与 `DELETE /api/agents/:agentId`。
  - 已新增 `apps/platform/src/pages/agents/AgentsManagementPage.tsx`，正式消费 `agents / knowledge / skills` 三组接口，展示状态、绑定结果、system prompt 摘要与固定模型信息。
  - 已补创建 / 编辑弹窗、删除确认与绑定多选交互，直接复用正式 `knowledge / skills` 目录作为绑定源。
  - 当前项目资源页中的 `agents` fallback 没有在本任务中同步替换，保持任务边界收敛。
- 建议子任务：
  - 复用现有资产壳层，但 Agent 页需要新增表单抽屉或弹窗。
  - 绑定列表应直接消费正式 `knowledge / skills` 接口。
  - `boundSkillIds` 只允许系统内置或已发布 Skill，避免草稿配置进入正式链路。
  - 先不开放项目级覆盖配置。
- 验收：
  - `/agents` 页面可完成“创建 Agent -> 绑定知识库 / Skill -> 列表回显”闭环。

### GA-12 DONE（2026-03-14）· 集成验证、回归检查与文档同步
...
- 已完成记录：
  - 已在仓库根新增 `pnpm verify:global-assets-foundation`，统一执行 `pnpm --filter api test`、`pnpm --filter indexer-py test` 与 `pnpm --filter platform check-types`，作为 Week 3-4 的最小自动验证入口。
  - 已补 `apps/api/src/modules/knowledge/knowledge.service.test.ts` 的 `searchDocuments` 检索测试，与既有 `uploadDocument / retryDocument / deleteDocument` 测试共同固定 Node 侧知识链路行为。
  - 已形成一条可复核的最小闭环验证记录：Node 建记录并触发 Python 由 `knowledge.service.test.ts` 的 `uploadDocument falls back to legacy indexer route after 404 on versioned path` 覆盖，Python API / Chroma 写侧契约由 `apps/indexer-py/tests/test_api.py::test_index_documents_success_response_keeps_existing_shape` 与 `apps/indexer-py/tests/test_pipeline.py::test_process_document_checks_collection_before_embedding` 覆盖，Node 查询入口由新增 `searchDocuments normalizes query filters and delegates to the shared search service` 覆盖；前端当前以 `pnpm --filter platform check-types` 作为最小 smoke，未额外引入浏览器交互自动化。
  - 已同步更新 `README.md`、`README.zh-CN.md`、`apps/api/README.md`、`apps/platform/README.md`、`.agent/docs/current/architecture.md` 与 `.agent/docs/roadmap/gap-analysis.md`，收口 `/skills`、`/agents` 已正式接线、项目资源页仍保留 fallback、`GlobalAssetManagementPage.tsx` 已退为历史壳层等当前事实。
  - 已明确延后项：`global_code` 真实导入、项目私有知识库、项目资源页 `agents` fallback 清理、`retry / rebuild / diagnostics` 正式 API、Skill runtime / Agent 执行编排，以及 Node 侧 Chroma delete 过渡 TODO 的最终收口。
- 建议子任务：
...
- 全局知识库稳定 ID 与列表接口。
- 全局 Skill 稳定 ID、hybrid registry 与 `draft / published` 生命周期。
...
- 风险 1：上传解析链路拖慢整个阶段
  - 止损：先保证 `md / markdown / txt`，`pdf` 延后到 `indexer-py` 正式覆盖后再恢复。
- 风险 2：Chroma 集成超预期复杂
...
- 风险 4：Skill 范围膨胀
  - 止损：治理范围已扩到自建 / 导入，但仍不做 runtime、回源同步和执行沙箱。
...
- [x] `/skills` 已展示正式 Skill 列表，且包含 3 个内置 Skill。
- [x] `/agents` 已支持创建并绑定知识库 / Skill。
...
Week 3-4 的正确打法不是“马上做完整 AI 对话”，而是先把全局资产正式化：知识库先跑通最短索引闭环，Skill 从 registry 扩到正式资产治理，Agent 先立配置模型。这样 Week 5-8 才有稳定地基可接。
