...
- GA-09 已完成：
  ...
  - 页面已标注“系统内置 / 可用状态 / 类型 / handler / 参数契约”，可支撑后续 Agent 绑定选择。
  - 当前阶段未开放新建 / 导入按钮，已改为只读目录说明态，不再保留假按钮。
- GA-10 已完成：
  - `agents` 模块已落地 Mongo 正式模型、列表 / 详情 / 创建 / 编辑 / 删除接口，以及 `name / systemPrompt / status` 等最小字段校验。
  - 服务端已校验 `boundKnowledgeIds` 必须命中已存在知识库、`boundSkillIds` 必须命中内置 Skill registry，避免绑定脏引用。
...
- 全局资产当前仍是部分前端 Mock：
  ...
  - `project.catalog.ts` 仍提供项目资源页与成员聚合所需的 `skills / agents` 过渡目录。
...
- 后端已建立 `knowledge / skills / agents` 正式模块边界；其中 `knowledge` 已具备真实元数据模型、CRUD、上传入口、`global_docs` Chroma 写入、统一检索与前端正式状态视图，`skills` 已完成代码 registry 与只读查询接口，`agents` 已完成后端正式模型、CRUD 与绑定校验。
...
### GA-09 DONE（2026-03-14）· 前端 `/skills` 接正式数据并标记内置能力
...
- 已完成记录：
  ...
  - 已新增 `apps/platform/src/pages/skills/SkillsManagementPage.tsx`，保留现有壳层语义，但改为从正式接口拉取 Skill 目录。
  - 页面已补齐“系统内置 / 只读目录”说明态，以及类型、状态、handler、参数契约与参数摘要展示，不再依赖 `project.catalog.ts` 的 Mock Skill 卡片。
...
- 已完成记录：
  ...
  - 已明确延后项：`global_code` 真实导入、项目私有知识库、项目资源页 `skills / agents` fallback 清理、`retry / rebuild / diagnostics` 正式 API、Skill runtime / Agent 执行编排，以及 Node 侧 Chroma delete 过渡 TODO 的最终收口。
- 建议子任务：
