...
- `docs/exports/` 是派生导出目标根目录，不承担事实源职责；导出包 live 入口为 `docs/exports/chatgpt-projects/*`。
- 项目级 Skill 的 live 根目录为 `.agents/skills/`；`.codex/` 仅保留兼容 stub 跳转。
...
docs/exports/ 派生导出目标根目录（非事实源，live 入口为 `docs/exports/chatgpt-projects/*`）
...
- 涉及以下变化时，必须同步检查并更新文档：
  ...
  - Root governance、工程治理 `standards/`、导出映射或 Skill 根目录变化：同步 `AGENTS.md`、`.codex/README.md`、`.codex/MIGRATION.md`、`docs/README.md`，必要时同步 `docs/exports/README.md`；导出包 active 引用统一使用 `docs/exports/chatgpt-projects/*`；Skill 根目录以 `.agents/skills/README.md` 为当前入口。
