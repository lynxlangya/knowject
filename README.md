...
- `/knowledge` and `/skills` are now wired to the formal backend asset APIs; `/skills` now centers on structured method-asset authoring (`preset|team`, `status`, `category`, `owner`, `definition`) and generated markdown preview instead of GitHub/raw import or publish-first lifecycle copy. Agent APIs remain available behind the scenes, but the standalone global `/agents` page is currently hidden in v1, and project create/edit or resource surfaces no longer expose agent-specific UI.
