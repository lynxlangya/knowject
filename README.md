...
The repository is currently in an active foundation stage: the product shell, authentication flow, project/member data flow, project resource binding persistence, project conversation read/write plus a backend assistant/runtime baseline, formal global knowledge/skill management flows, workspace settings management, and Docker deployment baseline are already implemented. Saved workspace LLM settings now directly drive the project chat MVP through a unified `chat/completions`-compatible runtime, and the project chat page now defaults to the formal SSE streaming route; richer chat citation UX, stream recovery polish, `global_code`, project-side asset consumption cleanup, and deeper retrieval/runtime workflows are still being built out. The standalone global `/agents` page is currently hidden in v1 while the agent product shape is still being reconsidered.

## Current Status

- `apps/platform` already provides the authenticated shell, project routes, member management UI, formal global knowledge/skill management pages, and the `/settings` workspace settings center. The standalone global `/agents` route is intentionally hidden in v1 for now.
...
- `/knowledge` and `/skills` are now wired to the formal backend asset APIs. The live `/skills` page is intentionally minimal: it only lists existing team skills, supports status changes and deletion, and no longer exposes create/view/edit drawers or preset-skill display. Agent APIs remain available behind the scenes, but the standalone global `/agents` page is currently hidden in v1, and project create/edit or resource surfaces no longer expose agent-specific UI.
...
- The locale migration now covers login, the sidebar account language entry, global pages, `/knowledge`, `/skills`, `/settings`, and the full main `/project/:projectId/*` shell (`layout / overview / chat / resources / members`), including project-side user-visible fallback/toast copy. Agent page copy remains in the codebase but the standalone `/agents` route is currently hidden.
