...
关联模块：`apps/api` / `apps/indexer-py` / `packages/request` / `docker` / `scripts` / MongoDB / Chroma / `.codex/docs`
...
- `.codex/docs/standards/code-structure-governance.md`（结构治理）：该标准定义了何为巨石文件、职责失真、拆分例外等边界，Milestone 0-5 中 `knowledge.service.ts`、`pipeline.py`、`projects.service.ts`、`compose`/`scripts` 脚本等大体量文件的拆分必须先对照此标准的触发矩阵判断是否满足拆分条件并记录例外。
- `.codex/docs/standards/config-security-governance.md`（配置与安全）：所有与 Docker、Mongo、Chroma、env/secrets/env_FILE 的改动（特别是 Milestone 6 的契约收口）都要依此复核，含新增/internal 路由、health/check 手段，应记录风险、同步文档、并使用该标准里的文档同步点确认 `docker/README.md`、`.codex/docs/current/docker-usage.md` 等已更新。
- `.codex/docs/standards/review-checklist.md`（评审清单）：Node/Python/Docker 相关改动需走评审清单的结构治理、配置与安全、文档同步等 checkpoint；每次触发“巨石文件”/“core config change”/“Docker env change”都要在 PR 描述中对照 review checklist 勾选并说明处理结果，以避免漏掉关键治理项。
...
- `.codex/docs/contracts/chroma-decision.md`
...
- `.codex/docs/README.md` 需要同步，是因为它本身维护 `plans/` 的阅读入口和索引。
...
目标：先定义哪些行为绝不能在重构中回归。
主要文件：`.codex/docs/plans/tasks-service-indexing-refactor.md`、`apps/api/src/modules/knowledge/*.test.ts`、`apps/indexer-py/tests/*`、`scripts/verify-*`。
...
主要文件：`apps/api/src/modules/projects/projects.service.ts`、`apps/api/src/modules/projects/projects.repository.ts`、`.codex/docs/plans/tasks-chat-core-week7-8.md`。
...
主要文件：`.dockerignore`、`docker/README.md`、`.codex/docs/README.md`、必要时 `.codex/docs/current/docker-usage.md`。
...
- 2026-03-18：Milestone 5 收尾（`SR-50` + `SR-51` 完成）
  ...
  - 已同步更新 `.codex/docs/plans/tasks-chat-core-week7-8.md` 的对话持久化模型策略，以及 `apps/api/README.md`、`.codex/docs/current/architecture.md` 中 `modules/projects` 的当前边界描述。
...
- 2026-03-18：Milestone 6 进展（`SR-60` 完成）
  ...
  - 已同步更新 `README.md`、`docker/README.md`、`apps/api/README.md`、`.codex/docs/current/docker-usage.md`、`.codex/docs/current/architecture.md`，并顺手修正了 `docker-usage.md` 中关于 Chroma “尚未进入正式知识检索链路”的过时表述。
...
- 2026-03-18：Milestone 6 进展（`SR-61` 完成）
  ...
  - 已同步更新 `AGENTS.md`、`README.md`、`docker/README.md`、`.codex/docs/current/architecture.md`、`.codex/docs/current/docker-usage.md`，明确 `scripts/lib/` 已成为当前脚本结构的一部分。
...
- 2026-03-18：Milestone 6 收尾（`SR-62` 完成）
  ...
  - `docker/README.md` 已退回为 Docker 命令入口与操作跳转页；`.codex/docs/current/docker-usage.md` 明确为 Docker 当前事实单一主源，不再双写拓扑、端口、secrets 契约与运行边界。
  - `.codex/docs/README.md`、`README.md`、`.codex/docs/current/architecture.md` 已同步更新入口说明；同时修正了 `docker-usage.md` 内残留的 Chroma 旧表述，避免同一文件前后对“是否进入正式知识检索链路”给出冲突结论。
